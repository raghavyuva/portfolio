...
import skilldev from '../static/skill.svg'
function RightHero() {
    const { role_description, skill_header, role_question, drinks } =
        ProfileDescribe;
    const [open, setopen] = useState(false);
    return (
        <div  id="skills" class="container flex shadow-2xl text-primary-text flex-col px-6 py-10 mx-auto space-y-6 lg:h-[32rem] lg:py-16 lg:flex-row lg:items-center">
            <div class="w-full lg:w-1/2">
                <div class="lg:max-w-lg">
                    ...
                    <div class="grid gap-6 mt-8 sm:grid-cols-3">
                        {
                            skills?.map((ele) => (
                                <SkillCard
                                    Logo={ele.icon}
            ...
            <div class="flex items-center justify-center w-full h-96 lg:w-1/2">
                <img class="object-cover w-full h-full max-w-2xl rounded-md"
                    src={skilldev} alt="glasses photo" />
            </div>
        </div >
    );
}
